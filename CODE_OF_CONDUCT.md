...
- Respecting differing viewpoints and lived experiences; a well-argued
  alternative is a gift, not a threat.
- Receiving critique with openness: look for what is accurate before defending
  what is comfortable.
- Prioritising the health of the project and its community over any single
...
`sakthai-agent` (v2) does not contain a license. Standard copyright applies, and all rights are reserved by the author (beer-sakthai). The codebase is maintained as a personal project, so external pull requests are not actively solicited (see [CONTRIBUTING.md](CONTRIBUTING.md)). Engagement through issues, discussions, and bug reports is valued all the same — and every person who engages thoughtfully deserves the same standards.
...
Participation here — through issues, discussions, and bug reports — is welcomed and valued. SakThai v2 operates under a proprietary view-only license; community members engage as contributors of ideas and feedback, not as co-owners of the codebase. Code contributions require a prior CLA discussion per `CONTRIBUTING.md`.

This distinction shapes expectations, not respect. Every person who engages thoughtfully with this project deserves the same standards regardless of whether their contribution is a bug report or a patch.
...
> For security vulnerabilities, use the separate process in
> [SECURITY.md](SECURITY.md) — do not route them through this channel.
