...
def test_check_env_comprehensive_diagnostic(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Verify check_env with a fully configured environment."""
    # 1. Setup paths
    sakthai_home = tmp_path / "sakthai_home"
    sakthai_home.mkdir()
    monkeypatch.setenv("SAKTHAI_HOME", str(sakthai_home))

    # 2. Setup environment variables
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-123")
    monkeypatch.setenv("OPENAI_API_KEY", "sk-openai-123")
    monkeypatch.setenv("GEMINI_API_KEY", "gemini-123")
    monkeypatch.setenv("SAKTHAI_GATEWAY_URL", "https://gateway.example.com")
    monkeypatch.setenv("SAKTHAI_MCP_TIMEOUT", "60")

    # 3. Setup skills dir
    custom_skills = tmp_path / "skills"
    custom_skills.mkdir()
    (custom_skills / "skill_a").mkdir()
    monkeypatch.setattr(config, "SKILLS_DIR", custom_skills)

    # 4. Setup memory DB
    with MemoryStore() as store:
        store.add_fact("fact 1")
    ...
    # Paths
    assert report["paths"]["sakthai_home"] == str(sakthai_home)
    assert report["paths"]["sakthai_home_exists"] is True
    assert report["paths"]["memory_db_exists"] is True

    # Env
    env = report["env"]
    assert env["ANTHROPIC_API_KEY"]["set"] is True
    assert env["OPENAI_API_KEY"]["set"] is True
    assert env["GEMINI_API_KEY"]["set"] is True
    assert env["SAKTHAI_GATEWAY_URL"]["set"] is True
    assert env["SAKTHAI_MCP_TIMEOUT"]["set"] is True

    # Memory
    assert report["memory"]["db_exists"] is True
    assert report["memory"]["db_writable"] is True
    assert report["memory"]["fact_count"] == 1
    ...
    assert report["auth"]["openai_ok"] is True
    assert report["auth"]["gateway_ok"] is True


def test_check_env_minimal_diagnostic(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Verify check_env with a minimal/unconfigured environment."""
    # 1. Clear all relevant env vars first
    all_vars = {**config.REQUIRED_ENV_VARS, **config.OPTIONAL_ENV_VARS}
    for var in all_vars:
        monkeypatch.delenv(var, raising=False)

    # 2. Setup empty home (after clearing so SAKTHAI_HOME stays set)
    sakthai_home = tmp_path / "empty_home"
    # Don't even create the directory yet
    monkeypatch.setenv("SAKTHAI_HOME", str(sakthai_home))

    # 3. Mock auth sources to be None since we cleared env vars and didn't setup files
    # (The actual auth helpers might look at other places like ~/.claude, so
    # we want to ensure they return None for this "minimal" test)
    monkeypatch.setenv("CLAUDE_CONFIG_DIR", str(tmp_path / "nonexistent_claude"))
    monkeypatch.setenv("GEMINI_HOME", str(tmp_path / "nonexistent_gemini"))

    # Execute
    report = config.check_env()

    # Assertions
    # ready should still be True because DB doesn't exist yet (lazy creation)
    assert report["ready"] is True

    # Paths
    assert report["paths"]["sakthai_home_exists"] is False
    assert report["paths"]["memory_db_exists"] is False

    # Env
    assert report["env"]["ANTHROPIC_API_KEY"]["set"] is False

    # Memory
    assert report["memory"]["db_exists"] is False

    # Auth
    assert report["auth"]["anthropic_ok"] is False
    assert report["auth"]["openai_ok"] is False
    assert report["auth"]["gateway_ok"] is False


def test_check_env_ready_independent_of_auth(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Verify that readiness does not depend on credentials."""
    # 1. Setup paths
    sakthai_home = tmp_path / "ready_test_home"
    sakthai_home.mkdir()
    monkeypatch.setenv("SAKTHAI_HOME", str(sakthai_home))

    # 2. Ensure NO credentials are set
    all_vars = {**config.REQUIRED_ENV_VARS, **config.OPTIONAL_ENV_VARS}
    for var in all_vars:
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setenv("CLAUDE_CONFIG_DIR", str(tmp_path / "no_claude"))

    # 3. Memory DB exists and is writable
    with MemoryStore() as store:
        store.add_fact("test")
    ...
    assert report["memory"]["db_exists"] is True
    assert report["memory"]["db_writable"] is True
    assert report["ready"] is True  # Should still be ready!
