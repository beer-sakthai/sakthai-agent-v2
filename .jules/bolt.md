...
## Test Fix: 2026-06-30
Fixed pre-existing broken test `tests/test_memory_sync.py::TestSyncMemoryViaHttp::test_https_url_accepted`.
The test was incorrectly passing the entire success message string (`Synced to HTTP endpoint: https://...`) to `urlparse`, which failed to extract the hostname.
The fix extracts the URL part from the success message before parsing.
