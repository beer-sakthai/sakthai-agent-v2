...
def check_git_status(repo_path):
    try:
        ...
        return {
            ...
            "remediation": "Check if git is installed and if this is a valid git repository.",
        }
...
def check_omp_files(repo_path):
    ...
    if not os.path.exists(omp_state_dir):
        return {
            "status": "FAIL",
            "details": ".omp/state/ directory does not exist.",
            "remediation": "Run '/oh-my-product:launch' to initialize state directory.",
        }
    ...
    if failed:
        return {
            ...
            "remediation": "Re-run /oh-my-product:launch to fix missing or unwritable state files.",
        }
    else:
        return {
            "status": "PASS",
            "details": "All OMP state files and directories exist and are writeable.",
        }
...
def check_yaml_integrity(repo_path):
    ...
    if failed:
        return {
            "status": "FAIL",
            "details": "; ".join(findings),
            "remediation": "Restore the config file from backup or fix the YAML syntax error manually.",
        }
    else:
        return {
            "status": "PASS",
            "details": "All configuration YAML files are valid and structurally sound.",
        }
...
def check_deployment_script(repo_path):
    ...
    if not os.path.exists(deploy_path):
        return {
            "status": "FAIL",
            "details": "deploy.py script is missing.",
            "remediation": "Re-run the previous agent step to generate deploy.py.",
        }
    if not os.access(deploy_path, os.X_OK):
        return {
            "status": "FAIL",
            "details": "deploy.py is not executable.",
            "remediation": "Run 'chmod +x deploy.py' to make it executable.",
        }
...
def check_systemd_files(repo_path):
    ...
    if not os.path.exists(systemd_src):
        return {
            "status": "FAIL",
            "details": "systemd directory is missing in the repository.",
            "remediation": "Restore systemd directory.",
        }
    ...
    if failed:
        return {
            "status": "FAIL",
            "details": "; ".join(findings),
            "remediation": "Restore the missing systemd service files in the systemd directory.",
        }
    else:
        return {
            "status": "PASS",
            "details": "All required systemd service definitions exist in the repository.",
        }
...
def main():
    ...
    output = {"overall_health": overall_health, "diagnostics": results}
